...
def Tools_Menu():
    ...
    Tools = [
        ...
        [sg.Text("Methods:")],
        [sg.Checkbox("TCP", key="TCP"),
         sg.InputOptionMenu(("HTTP", "Coming Soon..."), size=(15, 1), key="type_", default_value="HTTP")],
        [sg.Checkbox("UDP", key="UDP")],
        [sg.Checkbox("OVH", key="OVH")],
        [sg.ProgressBar(max_value=100, orientation='h', size=(40, 20), key='progress_1'),
         sg.Text("0%", key="progress_text"), sg.Text(f"Sent: {send_attack}", key="sent")],
        [sg.Output(size=(100, 10), key='log')],
        [sg.Button("Sent"), sg.Button("Clear Log", key="clear_log"), sg.Button("Exit")]
    ]

    windows = sg.Window('discord - xumae#0001', Tools, grab_anywhere=True, font=("Helvetica", 12),
                        auto_size_buttons=False, keep_on_top=True, disable_minimize=True)
